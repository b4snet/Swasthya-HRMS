...
export function NotFoundState({
  title = "Not found",
  message = "The record you requested does not exist or is outside your access reach.",
  className,
}: {
  title?: string;
  message?: string;
  ...
  return (
    <div
      role="status"
      className={cn(
        "flex flex-col items-start gap-2 rounded-lg border border-border bg-muted p-5",
        className,
      )}
    >
      <div className="flex items-center gap-2">
        <ShieldX aria-hidden="true" className="h-5 w-5 text-muted-foreground" />
        <p className="text-sm font-semibold text-foreground">{title}</p>
      </div>
