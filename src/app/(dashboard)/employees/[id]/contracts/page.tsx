...
export default async function ContractsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Contracts bind to employments; pass the primary (first) employment id.
    const primaryEmploymentId = profile.employments[0]?.id ?? null;
    if (!primaryEmploymentId) {
      return (
        <EmptyState
          title="No employment to contract"
          description="Contracts attach to an employment. Create the employment first."
        />
      );
    }
    return (
      <ContractsClient
        ...
        employmentId={primaryEmploymentId}
      />
    );
  } catch (err) {
